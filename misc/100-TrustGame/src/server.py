...
"""This program is free software: you can redistribute it and/or modify
   ...
   Customizations made by 0xdade for the BSidesPDX 2019 CTF.
"""
...
class MoguiServer(SocketServer.ThreadingMixIn, SocketServer.TCPServer):
    ...
    def __init__(self, server_address):
        ...
        self.buffer_size = 87380 #4194304 (19.04 max) #4096 (19.04 min) #16384 (19.04 default) # 87380 (14.04/16.04 default)

        # What to fill the tcp buffers with
        self.padding = '\x00' * (self.buffer_size)
        ...
        # HTTP HTML header
        self.packet_html = ("HTTP/1.1 200 OK\r\n" + \
                     "Server: Apache\r\n" + \
                     "Date: %s\r\n" + \
    ...
    def setscript(self, uri, params):
        ...
        (benign, null, good, flag, min_jump, max_variance) = params
        
        benign = open(benign, "r").read() # benign html file
        null = open(null, "r").read() # Base file with a delay
        good = open(good, "r").read() # Non malicious payload
        flag = open(flag, "r").read()   # Malicious payload

        self.payloads[uri] = (benign, null, good, flag, min_jump, max_variance)
...
class HTTPHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        self.log("Request for %s" % request_uri)
        ...
        (payload_benign, payload_plain, payload_good, payload_bad, min_jump, max_var) = self.server.payloads[request_uri]
        
        if not re.search("User-Agent: (curl|Wget)", data):
            self.request.sendall(self.server.packet_html)
            self.sendchunk(payload_benign)
            self.sendchunk("") # The empty chunk signals the client to close the connection
            self.log("Request not via wget/curl. Returning html page.")
            return

        if re.search("User-Agent: (curl)", data):
            self.request.sendall(self.server.packet_200)
            self.sendchunk("Booo. Curl stinks. Wget is superior.\n")
            self.sendchunk("")
            self.log("Requests by curl don't get to play")
            return
...
if __name__ == "__main__":
    ...
    SERVER.setscript("/", ("index.html", "ticker.sh", "good.sh", "flag.sh", 2.0, 0.1))
